...
from BFhelper import brute_force_tsp
# import multiprocessing - for use in timing if we can't figure out another way

def exact(adjacency_matrix, time):
    return brute_force_tsp(adjacency_matrix, time)
...
def main(inst, alg, time, seed):
    ...
    if alg == 'BF':
        quality, ids = exact(adjacency_matrix, time)
...
# python tsp.py -inst Berlin.tsp -alg BF -time 30 -seed 42   
# python tsp.py -inst Berlin.tsp -alg Approx -time 30 -seed 42  
